...
// Pre-compute the NEP413 base int prefix (computed once at module load)
const NEP413_BASE_INT = 2 ** 31 + 413;
const NEP413_PREFIX = Buffer.from(borshSerialize(BorshSchema.u32, NEP413_BASE_INT));
...
export function serializeIntent(
  ...
  if (standard !== SignStandardEnum.nep413) {
  ...
  // Use pre-computed prefix instead of serializing every time
  const combinedData = Buffer.concat([NEP413_PREFIX, payloadSerialized]);
